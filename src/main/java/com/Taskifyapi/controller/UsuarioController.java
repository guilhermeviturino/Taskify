...
public class UsuarioController {
    ...
    @GetMapping
    public ResponseEntity<Page<Usuario>> listarUsuarios(@PageableDefault(size=10, page = 0, sort = "nome", direction = Direction.DESC) Pageable paginacao) { //gerar paginas de usuários, para que nao fiquem todos em uma só tela.
        return ResponseEntity.status(HttpStatus.OK).body(usuarioRepository.findAll(paginacao));
        
        //http://localhost:8080/usuarios?size=1&page=1 (acessar paginas.)
        //http://localhost:8080/usuarios?size=1&page=1&sort=idUsuario,desc (consultar por ordem descendente ou ascendente)
    ...
    public Endereco obterEnderecoPeloCep(@PathVariable("cep") String cep) {
        ...
        return restTamplete.getForObject(url, Endereco.class);
    }
